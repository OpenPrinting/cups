...
static const char *const http_months[12] = {/* Months of the year */
                                            "Jan", "Feb", "Mar", "Apr",
                                            "May", "Jun", "Jul", "Aug",
                                            "Sep", "Oct", "Nov", "Dec"};
static const char *const http_states[] =
    {/* HTTP state strings */
     "HTTP_STATE_ERROR",          "HTTP_STATE_WAITING",
     "HTTP_STATE_OPTIONS",        "HTTP_STATE_GET",
     "HTTP_STATE_GET_SEND",       "HTTP_STATE_HEAD",
     "HTTP_STATE_POST",           "HTTP_STATE_POST_RECV",
     "HTTP_STATE_POST_SEND",      "HTTP_STATE_PUT",
     "HTTP_STATE_PUT_RECV",       "HTTP_STATE_DELETE",
     "HTTP_STATE_TRACE",          "HTTP_STATE_CONNECT",
     "HTTP_STATE_STATUS",         "HTTP_STATE_UNKNOWN_METHOD",
...
static const char *http_copy_decode(char *dst, const char *src, int dstsize,
                                    const char *term, int decode);
static char *http_copy_encode(char *dst, const char *src, char *dstend,
                              const char *reserved, const char *term,
                              int encode);
#ifdef HAVE_DNSSD
static void DNSSD_API
http_resolve_cb(DNSServiceRef sdRef, DNSServiceFlags flags,
...
#ifdef HAVE_AVAHI
static void http_client_cb(AvahiClient *client, AvahiClientState state,
                           void *simple_poll);
...
httpAssembleURI(http_uri_coding_t encoding, /* I - Encoding flags */
                ...
                const char *resource)       /* I - Resource */
{
  ...
  if (!strcmp(scheme, "geo") || !strcmp(scheme, "mailto") ||
      !strcmp(scheme, "tel"))
  {
    /*
  ...
  if (host)
  {
    ...
    else
    {
       ...
       * "reg-name" ABNF rule; anything <= SP or >= DEL plus % gets
...
http_uri_status_t                            /* O - URI status */
httpAssembleURIf(http_uri_coding_t encoding, /* I - Encoding flags */
                 char *uri,                  /* I - URI buffer */
                 int urilen,                 /* I - Size of URI buffer */
                 const char *scheme,         /* I - Scheme name */
                 const char *username,       /* I - Username */
                 ...
                 ...) /* I - Additional arguments as needed */
{
  ...
  if ((size_t)bytes >= sizeof(resource))
  {
    *uri = '\0';
    ...
    return (httpAssembleURI(encoding, uri, urilen, scheme, username, host, port,
                            resource));
}
                 ...
                 size_t bufsize)     /* I - Size of buffer */
{
  ...
  snprintf(data, sizeof(data), "%s:%d:%s:%d:%04x:%04x", server, port,
           name ? name : server, number, (unsigned)CUPS_RAND() & 0xffff,
           (unsigned)CUPS_RAND() & 0xffff);

  cupsHashData("md5", (unsigned char *)data, strlen(data), md5sum,
               sizeof(md5sum));
                   ...
                   int slen) /* I - Size of string buffer */
{
  ...
  snprintf(s, (size_t)slen, "%s, %02d %s %d %02d:%02d:%02d GMT",
           http_days[tdate.tm_wday], tdate.tm_mday, http_months[tdate.tm_mon],
...
void httpSeparate(const char *uri, /* I - Universal Resource Identifier */
...
{
  httpSeparateURI(HTTP_URI_CODING_ALL, uri, scheme, 32, username, HTTP_MAX_URI,
                  host, HTTP_MAX_URI, port, resource, HTTP_MAX_URI);
}
...
void httpSeparate2(const char *uri, /* I - Universal Resource Identifier */
...
httpSeparateURI(http_uri_coding_t decoding, /* I - Decoding flags */
                ...
                int resourcelen) /* I - Size of resource buffer */
{
  ...
  else
  {
    ...
    if (uri && *uri == '?')
    {
      ...
      uri =
          http_copy_decode(resptr, uri, resourcelen - (int)(resptr - resource),
...
_httpSetDigestAuthString(http_t *http,      /* I - HTTP connection */
                         ...
                         const char *resource) /* I - HTTP resource path */
{
  ...
  DEBUG_printf(("2_httpSetDigestAuthString(http=%p, nonce=\"%s\", "
                "method=\"%s\", resource=\"%s\")",
                (void *)http, nonce, method, resource));
  ...
  if (http->algorithm[0])
  {
    ...
    hashsize = (size_t)cupsHashData(hashalg, (unsigned char *)temp,
                                    strlen(temp), hash, sizeof(hash));
    ...
    snprintf(temp, sizeof(temp), "%s:%s:%08x:%s:%s:%s", ha1, http->nonce,
             http->nonce_count, cnonce, "auth", ha2);
    hashsize = (size_t)cupsHashData(hashalg, (unsigned char *)temp,
                                    strlen(temp), hash, sizeof(hash));
    ...
    if (http->opaque[0])
      snprintf(digest, sizeof(digest),
               "username=\"%s\", realm=\"%s\", nonce=\"%s\", algorithm=%s, "
               "qop=auth, opaque=\"%s\", cnonce=\"%s\", nc=%08x, uri=\"%s\", "
               "response=\"%s\"",
               cupsUser(), http->realm, http->nonce, http->algorithm,
               http->opaque, cnonce, http->nonce_count, resource, kd);
    else
      snprintf(digest, sizeof(digest),
               "username=\"%s\", realm=\"%s\", nonce=\"%s\", algorithm=%s, "
               "qop=auth, cnonce=\"%s\", nc=%08x, uri=\"%s\", response=\"%s\"",
               username, http->realm, http->nonce, http->algorithm, cnonce,
               http->nonce_count, resource, kd);
  }
  else
  {
    /*
    ...
    /* H(A1) = H(username:realm:password) */
    snprintf(temp, sizeof(temp), "%s:%s:%s", username, http->realm, password);
    hashsize = (size_t)cupsHashData("md5", (unsigned char *)temp, strlen(temp),
                                    hash, sizeof(hash));
    cupsHashString(hash, hashsize, ha1, sizeof(ha1));
    ...
    snprintf(digest, sizeof(digest),
             ...
             "response=\"%s\"",
             username, http->realm, http->nonce, resource, kd);
  }
...
const char *                                  /* O - Localized status string */
httpURIStatusString(http_uri_status_t status) /* I - URI status code */
{
  const char *s;                        /* Status string */
  _cups_globals_t *cg = _cupsGlobals(); /* Global data */
...
_cups_hstrerror(int error) /* I - Error number */
{
  static const char *const errors[] = /* Error strings */
      {"OK", "Host not found.", "Try again.", "Unrecoverable lookup error.",
...
_httpResolveURI(const char *uri,          /* I - DNS-SD URI */
                ...
                void *context)            /* I - Context pointer for callback */
{
  char scheme[32], /* URI components... */
      userpass[256], hostname[1024], resource[1024];
  int port;
#ifdef DEBUG
  ...
  DEBUG_printf(
      ("_httpResolveURI(uri=\"%s\", resolved_uri=%p, resolved_size=" CUPS_LLFMT
       ", options=0x%x, cb=%p, context=%p)",
       uri, (void *)resolved_uri, CUPS_LLCAST resolved_size, options,
       (void *)cb, context));
  ...
  if (strstr(hostname, "._tcp"))
  {
    ...
    for (domain = strchr(regtype, '.'); domain;
    ...
    DEBUG_printf(
        ("2_httpResolveURI: Resolving hostname=\"%s\", regtype=\"%s\", "
         "domain=\"%s\"\n",
    ...
    if (DNSServiceCreateConnection(&ref) == kDNSServiceErr_NoError)
    {
      ...
      localref = ref;
      if (DNSServiceResolve(&localref, kDNSServiceFlagsShareConnection,
                            myinterface, hostname, regtype, "local.",
                            http_resolve_cb, &uribuf) == kDNSServiceErr_NoError)
      {
        ...
        while (time(NULL) < end_time)
        {
          ...
          if (fds < 0)
          {
            ...
            {
              DEBUG_printf(
                  ("2_httpResolveURI: poll error: %s", strerror(errno)));
              break;
            }
          }
          else if (fds == 0)
          {
            ...
            if (extrasent == 0 && domain && _cups_strcasecmp(domain, "local."))
            {
              if (options & _HTTP_RESOLVE_STDERR)
                fprintf(stderr,
                        "DEBUG: Resolving \"%s\", regtype=\"%s\", "
                        "domain=\"%s\"...\n",
                        hostname, regtype, domain ? domain : "");
            ...
            else if (extrasent == 0 && !strcmp(scheme, "ippusb"))
            {
              if (options & _HTTP_RESOLVE_STDERR)
                fprintf(stderr,
                        "DEBUG: Resolving \"%s\", regtype=\"_ipps._tcp\", "
                        "domain=\"local.\"...\n",
                        hostname);

              ippsref = ref;
              if (DNSServiceResolve(&ippsref, kDNSServiceFlagsShareConnection,
            ...
            else if (extrasent == 1 && !strcmp(scheme, "ippusb"))
            {
              if (options & _HTTP_RESOLVE_STDERR)
                fprintf(stderr,
                        "DEBUG: Resolving \"%s\", regtype=\"_ipp._tcp\", "
                        "domain=\"local.\"...\n",
                        hostname);

              ippref = ref;
              if (DNSServiceResolve(&ippref, kDNSServiceFlagsShareConnection,
                                    kDNSServiceInterfaceIndexAny, hostname,
    ...
    if ((uribuf.poll = avahi_simple_poll_new()) != NULL)
    {
      ...
      if ((client = avahi_client_new(avahi_simple_poll_get(uribuf.poll), 0,
                                     http_client_cb, &uribuf, &error)) != NULL)
      {
        if (avahi_service_resolver_new(client, AVAHI_IF_UNSPEC,
                                       AVAHI_PROTO_UNSPEC, hostname, regtype,
                                       "local.", AVAHI_PROTO_UNSPEC, 0,
                                       http_resolve_cb, &uribuf) != NULL)
        {
              ...
              _cups_strcasecmp(domain, "local."))
          {
            ...
            avahi_service_resolver_new(
                client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, hostname, regtype,
                domain, AVAHI_PROTO_UNSPEC, 0, http_resolve_cb, &uribuf);
          }
...
static void http_client_cb(AvahiClient *client,    /* I - Client information */
...
static void DNSSD_API
http_resolve_cb(DNSServiceRef sdRef,            /* I - Service reference */
                ...
                void *context)                  /* I - Pointer to URI buffer */
{
  ...
  DEBUG_printf(("4http_resolve_cb(sdRef=%p, flags=%x, interfaceIndex=%u, "
                ...
                (void *)sdRef, flags, interfaceIndex, errorCode, fullName,
                hostTarget, port, txtLen, (void *)txtRecord, context));

  /*
   * If we have a UUID, compare it...
   */

  if (uribuf->uuid && (value = TXTRecordGetValuePtr(txtLen, txtRecord, "UUID",
                                                    &valueLen)) != NULL)
  {
  ...
  if ((value = TXTRecordGetValuePtr(txtLen, txtRecord, reskey, &valueLen)) !=
      NULL)
  {
    if (((char *)value)[0] == '/')
    {
    ...
    if ((addrlist = httpAddrGetList(hostTarget, AF_UNSPEC, fqdn)) != NULL)
    {
      for (addr = addrlist; addr; addr = addr->next)
      {
        int error = getnameinfo(&(addr->addr.addr),
                                (socklen_t)httpAddrLength(&(addr->addr)), fqdn,
  ...
  if ((!strcmp(scheme, "ipp") || !strcmp(scheme, "ipps")) &&
    ...
    httpAssembleURIf(HTTP_URI_CODING_ALL, uribuf->buffer, (int)uribuf->bufsize,
                     scheme, NULL, hostTarget, ntohs(port), "%s?snmp=false",
                     resource);
  else
    httpAssembleURI(HTTP_URI_CODING_ALL, uribuf->buffer, (int)uribuf->bufsize,
...
http_poll_cb(struct pollfd *pollfds,   /* I - File descriptors */
...
static void
http_resolve_cb(AvahiServiceResolver *resolver, /* I - Resolver (unused) */
                ...
                void *context)                /* I - Pointer to URI buffer */
{
  ...
  {
    /*
     * If this service is registered on loopback interface ("lo"), force the
     * host name to "localhost"...
     */

    if (uribuf->options & _HTTP_RESOLVE_STDERR)
      fputs("DEBUG: Service comes from loopback interface \"lo\", setting "
            "\"localhost\" as host name.\n",
            stderr);
    DEBUG_puts("Service comes from loopback interface \"lo\", setting "
               "\"localhost\" as host name.");
  ...
  httpAssembleURI(HTTP_URI_CODING_ALL, uribuf->buffer, (int)uribuf->bufsize,
                  scheme, NULL, hostTarget, port, resource);
  DEBUG_printf(("5http_resolve_cb: Resolved URI is \"%s\".", uribuf->buffer));
