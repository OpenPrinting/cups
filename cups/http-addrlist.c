...
httpAddrConnect(http_addrlist_t *addrlist, /* I - List of potential addresses */
                int *sock)                 /* O - Socket */
{
  DEBUG_printf(("httpAddrConnect(addrlist=%p, sock=%p)", (void *)addrlist,
                (void *)sock));
    ...
    int *cancel)               /* I - Pointer to "cancel" variable */
{
  ...
  DEBUG_printf(("httpAddrConnect2(addrlist=%p, sock=%p, msec=%d, cancel=%p)",
                (void *)addrlist, (void *)sock, msec, (void *)cancel));
  ...
  while (remaining > 0)
  {
    ...
    if (addrlist && nfds < (int)(sizeof(fds) / sizeof(fds[0])))
    {
      ...
      if ((fds[nfds] = (int)socket(httpAddrFamily(&(addrlist->addr)),
                                   SOCK_STREAM, 0)) < 0)
      {
        /*
      ...
      val = 1;
      setsockopt(fds[nfds], SOL_SOCKET, SO_REUSEADDR, CUPS_SOCAST & val,
                 sizeof(val));

#ifdef SO_REUSEPORT
      val = 1;
      setsockopt(fds[nfds], SOL_SOCKET, SO_REUSEPORT, CUPS_SOCAST & val,
                 sizeof(val));
#endif /* SO_REUSEPORT */

#ifdef SO_NOSIGPIPE
      val = 1;
      setsockopt(fds[nfds], SOL_SOCKET, SO_NOSIGPIPE, CUPS_SOCAST & val,
                 sizeof(val));
#endif /* SO_NOSIGPIPE */
      ...
      val = 1;
      setsockopt(fds[nfds], IPPROTO_TCP, TCP_NODELAY, CUPS_SOCAST & val,
                 sizeof(val));
      ...
      if (!connect(fds[nfds], &(addrlist->addr.addr),
                   (socklen_t)httpAddrLength(&(addrlist->addr))))
      {
    ...
    do
    {
      ...
      result = poll(pfds, (nfds_t)nfds,
                    addrlist ? 100 : remaining > 250 ? 250 : remaining);

      DEBUG_printf(
          ("1httpAddrConnect2: poll() returned %d (%d)", result, errno));
      ...
      timeout.tv_sec = 0;
      timeout.tv_usec =
          (addrlist ? 100 : remaining > 250 ? 250 : remaining) * 1000;

      result =
          select(max_fd + 1, &input_set, &output_set, &error_set, &timeout);

      DEBUG_printf(
          ("1httpAddrConnect2: select() returned %d (%d)", result, errno));
#endif /* HAVE_POLL */
    }
#ifdef _WIN32
...
#endif /* HAVE_POLL */
        {
          ...
          if (i < nfds)
          {
            memmove(fds + i, fds + i + 1,
                    (size_t)(nfds - i) * (sizeof(fds[0])));
            memmove(addrs + i, addrs + i + 1,
                    (size_t)(nfds - i) * (sizeof(addrs[0])));
          }
          i--;
        }
      }
...
httpAddrCopyList(http_addrlist_t *src) /* I - Source address list */
{
...
void httpAddrFreeList(http_addrlist_t *addrlist) /* I - Address list to free */
{
...
httpAddrGetList(const char *hostname, /* I - Hostname, IP address, or NULL for
                                         passive listen address */
                ...
                const char *service)  /* I - Service name or port number */
{
  ...
  if (hostname && hostname[0] == '/')
  {
    ...
    if ((first = (http_addrlist_t *)calloc(1, sizeof(http_addrlist_t))) != NULL)
    {
      ...
      strlcpy(first->addr.un.sun_path, hostname,
              sizeof(first->addr.un.sun_path));
    }
  }
  else
#endif /* AF_LOCAL */
    ...
    if (hostname)
    {
      ...
      if (!*ptr)
      {
            ...
            ip[0] <= 255 && ip[1] <= 255 && ip[2] <= 255 && ip[3] <= 255)
        {
          ...
          first->addr.ipv4.sin_addr.s_addr =
