               ...
               int namelen)             /* I - Size of name buffer */
{
  ...
  DEBUG_printf(("httpAddrLookup(addr=%p, name=%p, namelen=%d)", (void *)addr,
                (void *)name, namelen));
...
#ifdef HAVE_GETNAMEINFO
  {
    ...
    int error = getnameinfo(&addr->addr, (socklen_t)httpAddrLength(addr), name,
                            (socklen_t)namelen, NULL, 0, 0);
...
void _httpAddrSetPort(http_addr_t *addr, /* I - Address */
                      int port)          /* I - Port */
{
  if (!addr || port <= 0)
    return;
...
{
  DEBUG_printf(("httpAddrString(addr=%p, s=%p, slen=%d)", (void *)addr,
                (void *)s, slen));
  ...
  else if (addr->addr.sa_family == AF_INET6)
  {
...
#ifdef HAVE_GETNAMEINFO
    if (getnameinfo(&addr->addr, (socklen_t)httpAddrLength(addr), temps,
                    sizeof(temps), NULL, 0, NI_NUMERICHOST))
    {
      /*
    ...
    for (sptr = temps, i = 0; i < 4 && addr->ipv6.sin6_addr.s6_addr32[i]; i++)
    {
      ...
      snprintf(sptr, sizeof(temps) - (size_t)(sptr - temps), "%s%x", prefix,
               (temp >> 16) & 0xffff);
      prefix = ":";
      sptr += strlen(sptr);
    ...
    if (i < 4)
    {
      ...
      if (i < 4)
      {
        ...
        for (; i < 4; i++)
        {
          ...
          {
            snprintf(sptr, sizeof(temps) - (size_t)(sptr - temps), "%s%x",
                     prefix, (temp >> 16) & 0xffff);
            sptr += strlen(sptr);
          }
...
httpGetHostByName(const char *name) /* I - Hostname or IP address */
{
  ...
  if (!*nameptr)
  {
    ...
    cg->ip_addr = htonl(
        (((((((unsigned)ip[0] << 8) | (unsigned)ip[1]) << 8) | (unsigned)ip[2])
          << 8) |
         (unsigned)ip[3]));
