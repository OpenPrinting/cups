...
static ipp_uchar_t bad_collection2[] =	/* Collection buffer (bad encoding) */
		{
		/*
		 * Real-life example: Get-Printer-Attributes response
		 * from Pantum M7300FDW.
		 *
		 * This device doesn't use IPP_TAG_MEMBERNAME. Instead,
		 * it uses named attributes within collections. This
		 * is IPP specification violation, but we have to live
		 * with it :(
		 */
     ...
     char *argv[])		/* I - Command-line arguments */
{
  ...
  if (argc == 1)
  {
   ...
   /*
    * Test for Pantum M7300FDW workaround
    */
    fputs("Test for Pantum M7300FDW workaround:", stdout);

    request = ippNew();
    data.rpos    = 0;
    data.wused   = sizeof(bad_collection2);
    data.wsize   = sizeof(bad_collection2);
    data.wbuffer = bad_collection2;
