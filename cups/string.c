...
/*
 * 'cupsFormatString()' - Copy formatted string into a buffer.
 *
		 ...
		 ...)			 /* I - Additional arguments as needed */
{
  ...
  va_list	ap;			 /* Argument pointers */


  if (!dst || dst_len < 2 || !format)
    return (copied);

  memset(dst, 0, dst_len);

  va_start(ap, format);
  copied = _cups_safe_vsnprintf((char *)dst, dst_len, format, ap);
  va_end(ap);
