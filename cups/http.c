...
static void http_add_field(http_t *http, http_field_t field, const char *value,
                           int append);
#ifdef HAVE_LIBZ
...
static http_t *http_create(const char *host, int port,
                           ...
                           http_encryption_t encryption, int blocking,
                           _http_mode_t mode);
#ifdef DEBUG
...
static int http_send(http_t *http, http_state_t request, const char *uri);
...
static const char *const http_fields[] = {"Accept-Language",
                                          "Accept-Ranges",
                                          "Authorization",
                                          "Connection",
                                          "Content-Encoding",
                                          ...
                                          blocking, 0 otherwise */
{
  ...
  val = 1;
  setsockopt(http->fd, SOL_SOCKET, SO_NOSIGPIPE, CUPS_SOCAST & val,
             sizeof(val));
#endif /* SO_NOSIGPIPE */
  ...
  val = 1;
  setsockopt(http->fd, IPPROTO_TCP, TCP_NODELAY, CUPS_SOCAST & val,
             sizeof(val));
...
httpAddCredential(cups_array_t *credentials, /* I - Credentials array */
    ...
    cups_array_t *cred2) /* I - Second set of X.509 credentials */
{
  ...
  for (temp1                 = (http_credential_t *)cupsArrayFirst(cred1),
      temp2                  = (http_credential_t *)cupsArrayFirst(cred2);
       temp1 && temp2; temp1 = (http_credential_t *)cupsArrayNext(cred1),
      temp2                  = (http_credential_t *)cupsArrayNext(cred2))
...
httpConnect2(
    ...
    int port,         /* I - Port number */
    http_addrlist_t
        *addrlist, /* I - List of addresses or @code NULL@ to lookup */
    ...
    int *cancel) /* I - Pointer to "cancel" variable */
{
  ...
  DEBUG_printf(("httpConnect2(host=\"%s\", port=%d, addrlist=%p, family=%d, "
                "encryption=%d, blocking=%d, msec=%d, cancel=%p)",
                host, port, (void *)addrlist, family, encryption, blocking,
                msec, (void *)cancel));
...
httpConnectEncrypt(
    ...
    http_encryption_t encryption) /* I - Type of encryption to use */
{
  DEBUG_printf(("httpConnectEncrypt(host=\"%s\", port=%d, encryption=%d)", host,
                port, encryption));

  return (
      httpConnect2(host, port, NULL, AF_UNSPEC, encryption, 1, 30000, NULL));
}
...
void httpFlush(http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(("httpFlush(http=%p), state=%s", (void *)http,
...
httpFlushWrite(http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(("httpFlushWrite(http=%p) data_encoding=%d", (void *)http,
    ...
    cups_array_t *credentials) /* I - Array of credentials */
{
  ...
  for (credential = (http_credential_t *)cupsArrayFirst(credentials);
       credential; credential = (http_credential_t *)cupsArrayNext(credentials))
  {
...
httpGetContentEncoding(http_t *http) /* I - HTTP connection */
{
  ...
  if (http && http->fields[HTTP_FIELD_ACCEPT_ENCODING])
  {
    ...
    static const char *const codings[] = {/* Supported content codings */
                                          "deflate", "gzip", "x-deflate",
                                          "x-gzip"};
...
httpGetLength2(http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(("2httpGetLength2(http=%p), state=%s", (void *)http,
                httpStateString(http->state)));

  if (!http)
    return (-1);

  if (http->fields[HTTP_FIELD_TRANSFER_ENCODING] &&
      !_cups_strcasecmp(http->fields[HTTP_FIELD_TRANSFER_ENCODING], "chunked"))
  {
  ...
  else
  {
    ...
    if (!http->fields[HTTP_FIELD_CONTENT_LENGTH] ||
        !http->fields[HTTP_FIELD_CONTENT_LENGTH][0])
    {
      /*
      ...
      if (http->status >= HTTP_STATUS_MULTIPLE_CHOICES ||
          ...
          http->state == HTTP_STATE_DELETE || http->state == HTTP_STATE_TRACE ||
          http->state == HTTP_STATE_CONNECT)
        remaining = 0;
      else
        remaining = 2147483647;
    }
    else if ((remaining = strtoll(http->fields[HTTP_FIELD_CONTENT_LENGTH], NULL,
                                  10)) < 0)
      remaining = -1;

    DEBUG_printf(
        ("4httpGetLength2: content_length=" CUPS_LLFMT, CUPS_LLCAST remaining));
  }
         ...
         http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(("2httpGets(line=%p, length=%d, http=%p)", (void *)line, length,
                (void *)http));
  ...
  while (lineptr < lineend)
  {
    ...
    while (http->used == 0)
    {
      ...
      bytes = http_read(http, http->buffer + http->used,
                        (size_t)(HTTP_MAX_BUFFER - http->used));

      DEBUG_printf(
          ("4httpGets: read " CUPS_LLFMT " bytes.", CUPS_LLCAST bytes));
                 ...
                 int valuelen)       /* I - Size of value buffer */
{
  ...
  DEBUG_printf(("2httpGetSubField2(http=%p, field=%d, name=\"%s\", value=%p, "
                "valuelen=%d)",
                (void *)http, field, name, (void *)value, valuelen));

  if (value)
    *value = '\0';

  if (!http || !name || !value || valuelen < 2 || field <= HTTP_FIELD_UNKNOWN ||
      field >= HTTP_FIELD_MAX || !http->fields[field])
    return (NULL);
  ...
  for (fptr = http->fields[field]; *fptr;)
  {
    ...
    for (ptr = temp; *fptr && *fptr != '=' && !_cups_isspace(*fptr) &&
    ...
    *ptr = '\0';
    ...
    if (*fptr == '\"')
    {
      ...
      for (ptr    = value, fptr++; *fptr && *fptr != '\"' && ptr < end;
           *ptr++ = *fptr++)
        ;
         ...
         size_t length) /* I - Maximum number of bytes */
{
  ...
  DEBUG_printf(("httpPeek(http=%p, buffer=%p, length=" CUPS_LLFMT ")",
                (void *)http, (void *)buffer, CUPS_LLCAST length));
...
#ifdef HAVE_LIBZ
  if (http->used == 0 && (http->coding == _HTTP_CODING_IDENTITY ||
                          (http->coding >= _HTTP_CODING_GUNZIP &&
  ...
  if (http->coding >= _HTTP_CODING_GUNZIP)
  {
    ...
    if (http->used > 0 &&
        ((z_stream *)http->stream)->avail_in < HTTP_MAX_BUFFER)
    {
      ...
      if (((z_stream *)http->stream)->avail_in > 0 &&
        ...
        memmove(http->sbuffer, ((z_stream *)http->stream)->next_in,
                ((z_stream *)http->stream)->avail_in);
      ...
      memcpy(http->sbuffer + ((z_stream *)http->stream)->avail_in, http->buffer,
             buflen);
...
#ifdef DEBUG
      http_debug_hex("2httpPeek", (char *)http->sbuffer,
                     (int)((z_stream *)http->stream)->avail_in);
#endif /* DEBUG */
      ...
      if (http->used > 0)
  {
    ...
    DEBUG_printf(
        ("2httpPeek: grabbing %d bytes from input buffer...", (int)bytes));
           ...
           ...)                /* I - Additional args as needed */
{
  ...
  DEBUG_printf(
      ("2httpPrintf(http=%p, format=\"%s\", ...)", (void *)http, format));
  ...
  DEBUG_printf(
      ("3httpPrintf: (" CUPS_LLFMT " bytes) %s", CUPS_LLCAST bytes, buf));
          ...
          size_t length) /* I - Maximum number of bytes */
{
  ...
  DEBUG_printf(("httpRead2(http=%p, buffer=%p, length=" CUPS_LLFMT
                ...
                (void *)http, (void *)buffer, CUPS_LLCAST length, http->coding,
    ...
    do
    {
      if (((z_stream *)http->stream)->avail_in > 0)
      {
...
#ifdef DEBUG
          http_debug_hex("2httpRead2", (char *)http->sbuffer,
                         (int)((z_stream *)http->stream)->avail_in);
#endif /* DEBUG */
        ...
        DEBUG_printf(("2httpRead2: avail_in=%d, avail_out=%d, bytes=%d",
                      ((z_stream *)http->stream)->avail_in,
                      ((z_stream *)http->stream)->avail_out, (int)bytes));
      }
      else
        bytes = 0;
      ...
      {
        ssize_t buflen =
            HTTP_MAX_BUFFER - (ssize_t)((z_stream *)http->stream)->avail_in;
  ...
  {
    DEBUG_printf(
        ("1httpRead2: Reading up to %d bytes into buffer.", (int)length));
                ...
                size_t urilen) /* I - Size of URI buffer */
{
  ...
  DEBUG_printf(("httpReadRequest(http=%p, uri=%p, urilen=" CUPS_LLFMT ")",
                (void *)http, (void *)uri, CUPS_LLCAST urilen));
  ...
  *req_version++ = '\0';
  ...
  DEBUG_printf(
      ("1httpReadRequest: Set state to %s.", httpStateString(http->state)));
               ...
               int *cancel)  /* I - Pointer to "cancel" variable */
{
  ...
  DEBUG_printf(("httpReconnect2(http=%p, msec=%d, cancel=%p)", (void *)http,
                msec, (void *)cancel));
  ...
  if ((addr = httpAddrConnect2(http->addrlist, &(http->fd), msec, cancel)) ==
      NULL)
  {
    /*
     * Unable to connect...
    ...
    DEBUG_printf(
        ("1httpReconnect2: httpAddrConnect failed: %s", strerror(http->error)));
                         ...
                         const char *value)  /* I - Value */
{
  DEBUG_printf(("httpSetDefaultField(http=%p, field=%d(%s), value=\"%s\")",
                (void *)http, field, http_fields[field], value));
...
void httpSetField(http_t *http,       /* I - HTTP connection */
                  ...
                  const char *value)  /* I - Value */
{
  DEBUG_printf(("httpSetField(http=%p, field=%d(%s), value=\"%s\")",
                (void *)http, field, http_fields[field], value));
...
{
  DEBUG_printf(("httpSetLength(http=%p, length=" CUPS_LLFMT ")", (void *)http,
                CUPS_LLCAST length));
            ...
            http_status_t *status) /* O - Current HTTP status */
{
  ...
  DEBUG_printf(("_httpUpdate(http=%p, status=%p), state=%s", (void *)http,
                (void *)status, httpStateString(http->state)));
  ...
  if (line[0] == '\0')
  {
    ...
    switch (http->state)
    {
      ...
      case HTTP_STATE_PUT:
        ...
        DEBUG_printf(
            ("1_httpUpdate: Set state to %s.", httpStateString(http->state)));
    ...
    *value++ = '\0';
    ...
    else if ((field = httpFieldValue(line)) != HTTP_FIELD_UNKNOWN)
    {
      ...
      if (field == HTTP_FIELD_AUTHENTICATION_INFO)
        httpGetSubField2(http, HTTP_FIELD_AUTHENTICATION_INFO, "nextnonce",
                         http->nextnonce, (int)sizeof(http->nextnonce));
    }
#ifdef DEBUG
    else
...
httpUpdate(http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(("httpUpdate(http=%p), state=%s", (void *)http,
                httpStateString(http->state)));
          ...
          int usessl)   /* I - Use SSL context? */
{
  ...
  DEBUG_printf(
      ("4_httpWait(http=%p, msec=%d, usessl=%d)", (void *)http, msec, usessl));
  ...
  DEBUG_printf(
      ("5_httpWait: returning with nfds=%d, errno=%d...", nfds, errno));
           ...
           size_t length)      /* I - Number of bytes to write */
{
  ...
  if (http->coding == _HTTP_CODING_GZIP || http->coding == _HTTP_CODING_DEFLATE)
  {
    ...
    else
    {
      ...
      {
        DEBUG_printf(("1httpWrite2: avail_out=%d",
                      ((z_stream *)http->stream)->avail_out));
        ...
        DEBUG_printf(
            ("1httpWrite2: Writing intermediate chunk, len=%d", (int)slen));
      ...
      if (length > 0)
  {
    ...
    {
      DEBUG_printf(("2httpWrite2: Flushing buffer (wused=%d, length=" CUPS_LLFMT
                    ")",
    ...
    if ((length + (size_t)http->wused) <= sizeof(http->wbuffer) &&
    ...
    else
    {
      ...
      DEBUG_printf(
          ("2httpWrite2: Wrote " CUPS_LLFMT " bytes...", CUPS_LLCAST bytes));
    }
       ...
       http->data_remaining == 0))
  {
...
#ifdef HAVE_LIBZ
    if (http->coding == _HTTP_CODING_GZIP ||
        http->coding == _HTTP_CODING_DEFLATE)
      http_content_coding_finish(http);
#endif /* HAVE_LIBZ */
    ...
    DEBUG_printf(
        ("2httpWrite2: Changed state to %s.", httpStateString(http->state)));
  }
                  ...
                  http_status_t status) /* I - Status code */
{
  ...
  if (!http->fields[HTTP_FIELD_SERVER])
    httpSetField(http, HTTP_FIELD_SERVER,
                 http->default_fields[HTTP_FIELD_SERVER]
                     ? http->default_fields[HTTP_FIELD_SERVER]
                     : CUPS_MINIMAL);
  ...
  if (!http->fields[HTTP_FIELD_ACCEPT_ENCODING])
    httpSetField(http, HTTP_FIELD_ACCEPT_ENCODING,
                 http->default_fields[HTTP_FIELD_ACCEPT_ENCODING]
                     ? http->default_fields[HTTP_FIELD_ACCEPT_ENCODING]
                     :
#ifdef HAVE_LIBZ
      ...
      status == HTTP_STATUS_SWITCHING_PROTOCOLS)
  {
    ...
    if (old_remaining <= INT_MAX)
      ...
      http->_data_remaining = INT_MAX;
           ...
           http->state == HTTP_STATE_HEAD || http->state == HTTP_STATE_PUT ||
           http->state == HTTP_STATE_TRACE ||
           http->state == HTTP_STATE_CONNECT ||
           http->state == HTTP_STATE_STATUS)
  {
...
static void http_add_field(http_t *http,       /* I - HTTP connection */
                           ...
                           int append)         /* I - Append value? */
{
  ...
  if (append && field != HTTP_FIELD_ACCEPT_ENCODING &&
      field != HTTP_FIELD_ACCEPT_LANGUAGE &&
      field != HTTP_FIELD_ACCEPT_RANGES && field != HTTP_FIELD_ALLOW &&
      field != HTTP_FIELD_LINK && field != HTTP_FIELD_TRANSFER_ENCODING &&
      field != HTTP_FIELD_UPGRADE && field != HTTP_FIELD_WWW_AUTHENTICATE)
    append = 0;
  ...
  if (total < HTTP_MAX_VALUE && field < HTTP_FIELD_ACCEPT_ENCODING)
  {
    ...
    if (fieldlen)
    {
      ...
      snprintf(combined, sizeof(combined), "%s, %s", http->_fields[field],
               value);
      value = combined;
    }
...
#ifdef HAVE_LIBZ
  if (field == HTTP_FIELD_CONTENT_ENCODING &&
      http->data_encoding != HTTP_ENCODING_FIELDS)
  {
...
static void http_content_coding_finish(http_t *http) /* I - HTTP connection */
{
  ...
  DEBUG_printf(
      ("1http_content_coding_finishing: http->coding=%d", http->coding));
  ...
  switch (http->coding)
  {
    ...
    case _HTTP_CODING_GZIP:
      ...
      do
      {
        ...
        {
          DEBUG_printf(
              ("1http_content_coding_finish: Writing trailing chunk, len=%d",
               (int)bytes));
...
static void
http_content_coding_start(http_t *http,      /* I - HTTP connection */
                          const char *value) /* I - Value of Content-Encoding */
{
  int zerr;              /* Error/status */
  _http_coding_t coding; /* Content coding value */

  DEBUG_printf(("http_content_coding_start(http=%p, value=\"%s\")",
                (void *)http, value));

  if (http->coding != _HTTP_CODING_IDENTITY)
  {
    DEBUG_printf(
        ("1http_content_coding_start: http->coding already %d.", http->coding));
    return;
  }
  ...
  switch (coding)
  {
    ...
    case _HTTP_CODING_GZIP:
      ...
      if ((zerr = deflateInit2((z_stream *)http->stream, Z_DEFAULT_COMPRESSION,
                               Z_DEFLATED,
    ...
    case _HTTP_CODING_GUNZIP:
      ...
      if ((zerr = inflateInit2((z_stream *)http->stream,
  ...
  DEBUG_printf(
      ("1http_content_coding_start: http->coding now %d.", http->coding));
}
#endif /* HAVE_LIBZ */
...
http_create(const char *host,             /* I - Hostname */
            int port,                     /* I - Port number */
            ...
            _http_mode_t mode)            /* I - _HTTP_MODE_CLIENT or _SERVER */
{
  ...
  DEBUG_printf(("4http_create(host=\"%s\", port=%d, addrlist=%p, family=%d, "
                "encryption=%d, blocking=%d, mode=%d)",
                host, port, (void *)addrlist, family, encryption, blocking,
                mode));
...
static void http_debug_hex(const char *prefix, /* I - Prefix for line */
          ...
          size_t length) /* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read(http=%p, buffer=%p, length=" CUPS_LLFMT ")",
                (void *)http, (void *)buffer, CUPS_LLCAST length));
                   ...
                   size_t length) /* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read_buffered(http=%p, buffer=%p, length=" CUPS_LLFMT
                ") used=%d",
                (void *)http, (void *)buffer, CUPS_LLCAST length, http->used));
  ...
  if (http->used > 0)
  {
    ...
    DEBUG_printf(
        ("2http_read: Grabbing %d bytes from input buffer.", (int)bytes));
                ...
                size_t length) /* I - Maximum bytes to read */
{
  ...
  if (http->data_remaining <= 0)
  {
    ...
    {
      DEBUG_printf(
          ("1http_read_chunk: Negative chunk length \"%s\" (" CUPS_LLFMT ")",
          ...
          const char *uri)      /* I - URI */
{
  ...
  static const char *const codes[] = /* Request code strings */
      {NULL,  "OPTIONS", "GET",    NULL,    "HEAD",  "POST", NULL, NULL,
       "PUT", NULL,      "DELETE", "TRACE", "CLOSE", NULL,   NULL};

  DEBUG_printf(("4http_send(http=%p, request=HTTP_%s, uri=\"%s\")",
                (void *)http, codes[request], uri));
  ...
  if (!http->fields[HTTP_FIELD_USER_AGENT])
  {
    if (http->default_fields[HTTP_FIELD_USER_AGENT])
      httpSetField(http, HTTP_FIELD_USER_AGENT,
                   http->default_fields[HTTP_FIELD_USER_AGENT]);
    else
...
http_set_length(http_t *http) /* I - Connection */
{
  ...
  DEBUG_printf(("http_set_length(http=%p) mode=%d state=%s", (void *)http,
                http->mode, httpStateString(http->state)));

  if ((remaining = httpGetLength2(http)) >= 0)
  {
    if (http->mode == _HTTP_MODE_SERVER && http->state != HTTP_STATE_GET_SEND &&
        http->state != HTTP_STATE_PUT && http->state != HTTP_STATE_POST &&
        http->state != HTTP_STATE_POST_SEND)
    {
    ...
    if (!_cups_strcasecmp(httpGetField(http, HTTP_FIELD_TRANSFER_ENCODING),
                          "chunked"))
    {
...
static void http_set_timeout(int fd,         /* I - File descriptor */
...
static void http_set_wait(http_t *http) /* I - HTTP connection */
{
  if (http->blocking)
  {
...
http_tls_upgrade(http_t *http) /* I - HTTP connection */
{
  ...
  if (!http->tls)
  {
    ...
    _cupsSetError(IPP_STATUS_ERROR_CUPS_PKI, _("Encryption is not supported."),
                  1);
    httpAddrClose(NULL, http->fd);
           ...
           size_t length)      /* I - Number of bytes to write */
{
  ...
  DEBUG_printf(("2http_write(http=%p, buffer=%p, length=" CUPS_LLFMT ")",
                (void *)http, (void *)buffer, CUPS_LLCAST length));
  http->error = 0;
  tbytes      = 0;
  ...
  while (length > 0)
  {
    ...
    if (http->timeout_value > 0.0)
    {
      ...
      do
      {
        ...
        else if (nfds == 0 && (!http->timeout_cb ||
    ...
    DEBUG_printf(("3http_write: Write of " CUPS_LLFMT
                  " bytes returned " CUPS_LLFMT ".",
                  CUPS_LLCAST length, CUPS_LLCAST bytes));
    ...
    if (bytes < 0)
    {
      ...
      DEBUG_printf(
          ("3http_write: error writing data (%s).", strerror(http->error)));
